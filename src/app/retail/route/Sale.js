...
export default class Sale extends PureComponent {
  ...
  handleGoodsClick = (record) => {
    const { invoice } = this.props.sale;
    // 如果已选清单中存在忽略
    let hasAdded = invoice.find((item) =>
      item.id === record.id
    );
    if(hasAdded) return;
  ...
  renderItem = (data) => {
    return data.map((item) => {
      return <Card.Grid key={item.id} className={style.gridStyle} onClick={() => this.handleGoodsClick(item)}>{item.title}</Card.Grid>;
    });
  };
  ...
  render() {
    ...
    const columns = [{
      ...
      title: '移除',
      render: (text, record) => {
        return <Icon type="delete" onClick={() => this.handleRemove(record.id)} className={style.removeBtn}/>;
      }
    }];
            ...
            <Col span={8} className={style.fullHeightCol}>
              ...
              <Row className={style.footer_row}>
                ...
                <Col className={style.footbar_right} span={6}>
                  <Button type='primary' className={style.checkOutBtn} onClick={() => this.handleCashBtnClick()}>收钱</Button>
                </Col>
              </Row>
            </Col>
          </Row>
